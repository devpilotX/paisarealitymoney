  ...
  return (
    <div>
      ...
      <div className="my-6">
        ...
        <p className="text-sm text-gray-500 mb-4">Based on {a.stats.years} years of bundled INR gold data (as of {GOLD_DATASET_AS_OF}). Past patterns do not predict the future.</p>
      ...
      <div className="card my-6 overflow-x-auto">
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {a.rolling.map((r) => (
              <tr key={r.windowYears} className="border-b border-gray-100">
                <td className="py-2 text-gray-700">{r.windowYears} year{r.windowYears > 1 ? 's' : ''}</td>
        ...
        {series.length > 0 ? (
          <>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mt-4 text-sm">
              <div><span className="text-gray-500 block text-xs">Total invested</span><strong>{formatCompactINR(a.backtest.totalInvested)}</strong></div>
              {a.backtest.sipFinalValue > 0 && <div><span className="text-gray-500 block text-xs">SIP final value</span><strong className="text-primary">{formatCompactINR(a.backtest.sipFinalValue)}</strong></div>}
              {a.backtest.lumpsumFinalValue > 0 && <div><span className="text-gray-500 block text-xs">Lump-sum final value</span><strong className="text-amber-700">{formatCompactINR(a.backtest.lumpsumFinalValue)}</strong></div>}
            </div>
          </>
        ) : <p className="text-sm text-gray-400 py-6 text-center">Enter a yearly SIP or lump sum to see the historical backtest.</p>}
      </div>
      ...
      <div className="card my-6 overflow-x-auto">
        ...
        <p className="text-xs text-gray-500 mb-3">Tax as of FY 2025-26. Long-term gains taxed at 12.5% + 4% cess; short-term at your slab. Verify current rules before acting.</p>
        <table className="w-full text-sm">
          <thead><tr className="text-left text-gray-500 border-b border-gray-200"><th className="py-2 font-medium">Instrument</th><th className="py-2 font-medium">Long-term after</th><th className="py-2 font-medium">Notes</th></tr></thead>
          <tbody>
            {a.allInstruments.map((inst) => (
              <tr key={inst.instrument} className={`border-b border-gray-100 ${inst.instrument === inputs.instrument ? 'bg-primary-50' : ''}`}>
                <td className="py-2 font-medium text-gray-800">{inst.label}</td>
                <td className="py-2 text-gray-600">{inst.longTermMonths} months → {inst.longTermRatePct}%</td>
                <td className="py-2 text-gray-600"><div>{inst.extra}</div><div className={`text-xs mt-0.5 ${inst.instrument === 'digital' ? 'text-red-600 font-medium' : 'text-gray-400'}`}>{inst.sebiCaveat}</div></td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <div className="card my-6">
        ...
        <ul className="space-y-2 list-disc pl-5">
          {a.educationalNotes.map((n, i) => <li key={i} className="text-sm text-gray-700">{n}</li>)}
        </ul>
      </div>
