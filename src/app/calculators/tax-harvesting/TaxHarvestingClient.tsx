    ...
    <label className="block">
      ...
      <div className="relative">
        ...
        <input type="number" value={Number.isFinite(value) ? value : 0} min={0} step={step}
          ...
          className={`w-full ${prefix ? 'pl-6' : 'pl-2'} pr-2 py-2 border border-gray-300 rounded-lg text-sm min-h-[40px] focus:outline-none focus:ring-2 focus:ring-primary`} />
      </div>
    </label>
  );
}
...
const ACTION_STYLE: Record<HarvestAction['type'], { tone: string; label: string }> = {
  ...
  wait: { tone: 'border-l-4 border-amber-400 bg-amber-50', label: 'Wait' },
  avoid: { tone: 'border-l-4 border-gray-300 bg-gray-50', label: 'Avoid' },
};

const TERM_BADGE: Record<string, string> = {
  long: 'bg-green-100 text-green-800',
  short: 'bg-amber-100 text-amber-800',
  debt: 'bg-indigo-100 text-indigo-800',
};
  ...
  return (
    <div>
          ...
          {inputs.lots.map((l) => (
            <div key={l.id} className="border border-gray-200 rounded-lg p-3">
              <div className="flex items-center gap-2 mb-2">
                <input type="text" value={l.name} onChange={(e) => updateLot(l.id, { name: e.target.value })} aria-label="Holding name" className="flex-1 px-2 py-2 border border-gray-300 rounded-lg text-sm font-medium min-h-[40px] focus:outline-none focus:ring-2 focus:ring-primary" />
                <select value={l.assetType} onChange={(e) => updateLot(l.id, { assetType: e.target.value as AssetType })} aria-label="Asset type" className="px-2 py-2 border border-gray-300 rounded-lg text-sm min-h-[40px] focus:outline-none focus:ring-2 focus:ring-primary">
                  <option value="equity">Equity</option>
                  <option value="debt">Debt</option>
                </select>
                {inputs.lots.length > 1 && <button type="button" onClick={() => removeLot(l.id)} aria-label={`Remove ${l.name}`} className="text-gray-400 hover:text-red-600 px-2 min-h-[40px] text-lg">, </button>}
              </div>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                <label className="block"><span className="block text-xs text-gray-500 mb-0.5">Buy date</span>
                  <input type="date" value={l.buyDate} onChange={(e) => updateLot(l.id, { buyDate: e.target.value })} className="w-full px-2 py-2 border border-gray-300 rounded-lg text-sm min-h-[40px] focus:outline-none focus:ring-2 focus:ring-primary" /></label>
      ...
      <details className="card mb-6">
        <summary className="cursor-pointer font-semibold text-gray-900 select-none">Already realized this year, carried-forward losses &amp; settings</summary>
        <div className="mt-4 grid grid-cols-2 sm:grid-cols-3 gap-3">
          ...
          <label className="block"><span className="block text-xs text-gray-500 mb-0.5">As of date</span>
            <input type="date" value={inputs.currentDate} onChange={(e) => setField('currentDate', e.target.value)} className="w-full px-2 py-2 border border-gray-300 rounded-lg text-sm min-h-[40px] focus:outline-none focus:ring-2 focus:ring-primary" /></label>
        </div>
      </details>

      {/* Headline */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        <div className="card bg-green-50 border-green-200"><p className="text-xs uppercase tracking-wide text-gray-500 mb-1">Tax saved this year</p><p className="text-2xl font-bold text-green-700">{formatCompactINR(a.taxSaved)}</p><p className="text-xs text-gray-500 mt-1">By harvesting losses to offset gains</p></div>
        <div className="card"><p className="text-xs uppercase tracking-wide text-gray-500 mb-1">Tax-free gains to harvest</p><p className="text-2xl font-bold text-primary">{formatCompactINR(a.gainHarvestAmount)}</p><p className="text-xs text-gray-500 mt-1">Basis reset worth ~{formatCompactINR(a.basisStepUpBenefit)} in future tax</p></div>
        <div className="card"><p className="text-xs uppercase tracking-wide text-gray-500 mb-1">Exemption headroom left</p><p className="text-2xl font-bold text-primary">{formatCompactINR(a.exemptionHeadroomRemaining)}</p><p className="text-xs text-gray-500 mt-1">of the ₹1.25L LTCG exemption</p></div>
      </div>
      ...
      <div className="card mb-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="text-center p-3 rounded-lg bg-gray-50"><p className="text-xs text-gray-500 mb-1">Without harvesting</p><p className="text-xl font-bold text-gray-900">{formatINR(a.baseline.tax)}</p></div>
          <div className="text-center p-3 rounded-lg bg-green-50"><p className="text-xs text-gray-500 mb-1">With harvesting</p><p className="text-xl font-bold text-green-700">{formatINR(a.afterHarvest.tax)}</p></div>
        </div>
      ...
      <div className="card my-6">
        ...
        <div className="space-y-2">
          {a.actions.length === 0 && <p className="text-sm text-gray-500">No harvesting actions. your lots are all short-term gains or there is nothing to optimise right now.</p>}
          {a.actions.map((act, i) => (
            <div key={i} className={`rounded-lg p-3 ${ACTION_STYLE[act.type].tone}`}>
              <div className="flex items-center gap-2 mb-0.5">
                <span className="text-xs font-semibold uppercase tracking-wide text-gray-600">{ACTION_STYLE[act.type].label}</span>
                <span className="text-sm font-medium text-gray-900">{act.lotName}</span>
              </div>
          ...
          <tbody>
            {a.classified.map((c) => (
              <tr key={c.id} className="border-b border-gray-100">
                ...
                <td className="py-2 text-center"><span className={`text-xs font-medium px-2 py-0.5 rounded ${TERM_BADGE[c.term]}`}>{c.term === 'long' ? 'LTCG' : c.term === 'short' ? 'STCG' : 'Debt'}</span>{c.nearBoundary ? <span className="ml-1 text-xs text-amber-600">~{c.daysToLongTerm}d to LT</span> : null}</td>
