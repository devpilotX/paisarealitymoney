...
const STATUS_STYLE: Record<string, { tone: string; title: string }> = {
  deficit: { tone: 'bg-red-50 border-red-200', title: '⚠️ You are in a monthly deficit' },
  tight: { tone: 'bg-amber-50 border-amber-200', title: '😬 Things are tight. savings below target' },
...
function SplitBar({ label, needs, wants, savings, total }: { label: string; needs: number; wants: number; savings: number; total: number }): React.ReactElement {
  ...
  return (
    <div className="mb-3">
      <div className="flex justify-between text-xs text-gray-500 mb-1"><span>{label}</span><span>{formatCompactINR(total)}</span></div>
      <div className="flex h-5 rounded-md overflow-hidden bg-gray-100" role="img" aria-label={`${label}: needs ${Math.round(pct(needs))}%, wants ${Math.round(pct(wants))}%, savings ${Math.round(pct(savings))}%`}>
        <div style={{ width: `${pct(needs)}%` }} className="bg-amber-400" title={`Needs ${formatCompactINR(needs)}`} />
        <div style={{ width: `${pct(wants)}%` }} className="bg-indigo-400" title={`Wants ${formatCompactINR(wants)}`} />
  ...
  return (
    <div>
      ...
      <details className="card mt-4" open>
        <summary className="cursor-pointer font-semibold text-gray-900 select-none">Your monthly spends</summary>
        <div className="mt-4 space-y-4">
          <div>
            <p className="text-xs font-semibold text-amber-700 uppercase tracking-wide mb-2">Needs</p>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
              {needsCats.map((c) => <NumField key={c.key} label={c.label} value={c.amount} onChange={(v) => setCategoryAmount(c.key, v)} />)}
            </div>
          </div>
          <div>
            <p className="text-xs font-semibold text-indigo-700 uppercase tracking-wide mb-2">Wants</p>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
      ...
      <details className="card mt-4">
        <summary className="cursor-pointer font-semibold text-gray-900 select-none">Your goals</summary>
        <div className="mt-4 space-y-3">
          {inputs.goals.map((g, i) => (
            <div key={i} className="grid grid-cols-2 sm:grid-cols-4 gap-2 items-end">
              <label className="block"><span className="block text-xs text-gray-500 mb-0.5">Goal</span>
                <input type="text" value={g.name} onChange={(e) => setGoal(i, { name: e.target.value })} className="w-full px-2 py-2 border border-gray-300 rounded-lg text-sm min-h-[40px] focus:outline-none focus:ring-2 focus:ring-primary" /></label>
      ...
      <div className="card my-6">
        ...
        <div className="flex flex-wrap gap-4 text-xs text-gray-500 mt-2">
          <span className="inline-flex items-center gap-1.5"><span className="w-3 h-3 rounded bg-amber-400" />Needs</span>
          <span className="inline-flex items-center gap-1.5"><span className="w-3 h-3 rounded bg-indigo-400" />Wants</span>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 my-6">
        <div className="card"><p className="text-xs uppercase tracking-wide text-gray-500 mb-1">Monthly surplus</p><p className="text-2xl font-bold text-primary">{formatCompactINR(analysis.monthlySurplus)}</p><p className="text-xs text-gray-500 mt-1">Capacity to save &amp; invest</p></div>
        <div className="card"><p className="text-xs uppercase tracking-wide text-gray-500 mb-1">Surplus found by trimming</p><p className="text-2xl font-bold text-green-700">{formatCompactINR(analysis.surplusFound)}</p><p className="text-xs text-gray-500 mt-1">If overspends are cut to benchmark</p></div>
        <div className="card"><p className="text-xs uppercase tracking-wide text-gray-500 mb-1">Emergency fund gap</p><p className="text-2xl font-bold text-primary">{formatCompactINR(analysis.emergencyFund.gap)}</p><p className="text-xs text-gray-500 mt-1">{analysis.emergencyFund.gap <= 0 ? `Fully funded (${analysis.emergencyFund.recommendedMonths} mo)` : analysis.emergencyFund.monthsToFill === Infinity ? 'No surplus to fill it yet' : `~${analysis.emergencyFund.monthsToFill} months to fill`}</p></div>
      </div>
          ...
          <div className="space-y-2">
            {analysis.overspendFlags.map((f) => (
              <div key={f.key} className="flex justify-between items-center text-sm">
                ...
                <span className="text-gray-500">{formatCompactINR(f.actual)} <span className="text-gray-400">vs</span> {formatCompactINR(f.benchmark)} <span className="font-semibold text-red-600">(+{formatCompactINR(f.overBy)})</span></span>
              </div>
            ))}
          </div>
        </div>
      ...
      {analysis.goals.length > 0 && (
        <div className="card my-6 overflow-x-auto">
          ...
          <table className="w-full text-sm">
            <thead><tr className="text-left text-gray-500 border-b border-gray-200"><th className="py-2 font-medium">Goal</th><th className="py-2 font-medium text-right">Needed/mo</th><th className="py-2 font-medium text-center">On track?</th></tr></thead>
            <tbody>
              {analysis.goals.map((g, i) => (
                <tr key={i} className="border-b border-gray-100">
                  <td className="py-2 text-gray-700">{g.name}</td>
      ...
      <div className="card my-6">
        ...
        <ol className="space-y-2 list-decimal pl-5">
          {analysis.actionList.map((a, i) => <li key={i} className="text-sm text-gray-700">{a}</li>)}
        </ol>
      </div>
