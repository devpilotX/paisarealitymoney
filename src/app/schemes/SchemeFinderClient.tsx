...
interface SchemeFinderClientProps {
  /** Live count of active schemes, passed from the server component. */
  schemeCount: number;
}

export default function SchemeFinderClient({ schemeCount }: SchemeFinderClientProps): React.ReactElement {
  const [schemes, setSchemes] = useState<MatchedScheme[]>([]);
  const [totalBenefit, setTotalBenefit] = useState<number>(0);
  const [isLoading, setIsLoading] = useState<boolean>(false);
  const [hasSearched, setHasSearched] = useState<boolean>(false);

  // Count is interpolated from the database so this copy cannot drift again.
  const countPhrase = schemeCount > 0 ? `${schemeCount}` : 'hundreds of';
  const schemeFaqs = [
