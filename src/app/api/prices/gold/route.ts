...
interface GoldPriceRow extends QueryResultRow {
  city_name: string; city_slug: string; state: string; price_date: string;
  gold_24k_per_gram: number; gold_22k_per_gram: number; gold_18k_per_gram: number;
  gold_24k_per_10gram: number; gold_22k_per_10gram: number; change_amount: number; change_percent: number;
}
      ...
      const data = await cacheGetOrFetch<GoldPriceRow[]>('gold-prices', cacheKey, async () => {
        return query<GoldPriceRow>(
          `SELECT c.name AS city_name, c.slug AS city_slug, c.state,
           ...
           FROM gold_prices gp JOIN cities c ON gp.city_id = c.id
           WHERE c.slug = $1 ORDER BY gp.price_date DESC LIMIT $2`,
          [sanitized, days]
        );
      }, { ttlMinutes: 15 });
